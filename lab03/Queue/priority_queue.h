...
struct Pair {
    T value;
    int priority;   // from 0 to N, 0 - highest
    Pair(T _val, int _prior);  // by default sets the highest priority
...
//    for(int i = 0; i < item_.Size(); i++) {  // ~ O(n^2)
//        item_[i].value.Print(); std::cout << '\n';
//    }
    ListNode<Pair<T>>* temp = item_.Begin();
