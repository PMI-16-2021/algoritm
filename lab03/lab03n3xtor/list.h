...
public:
    ...
    private:
        int index_;
    public:
        BadIndex(int index) : index_(index) { cout << "Error. Index " << index_ << " is invalid\n"; }
    };
};
...
template<typename T>
T& List<T>::Front() const { return head_->value;}

template<typename T>
T& List<T>::Back() const { return tail_->value; }

template<typename T>
bool List<T>::IsEmpty() const { return head_ == nullptr && tail_ == nullptr; }
